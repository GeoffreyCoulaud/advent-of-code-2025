...
def count_adjacent_rolls(grid: list[str], x_center: int, y_center: int) -> int:
    ...
    for x_offset, y_offset in iter_offsets():
        ...
        neighbor = grid[y][x]
        print(
            "(%d,%d) (%d,%d) => %s" % (x_center, y_center, x_offset, y_offset, neighbor)
        )
        if neighbor == "@":
            counter += 1
    return counter
...
def main() -> None:
    ...
    acessible_coords = find_accessible_coords(grid)
    acessible_count = len(acessible_coords)

    print("Total accessible: %d" % acessible_count)

    for y, line in enumerate(grid):
        printed_line = ""
        for x, slot in enumerate(line):
            if (x, y) in acessible_coords:
                printed_line += "x"
            else:
                printed_line += slot
        print(printed_line)
